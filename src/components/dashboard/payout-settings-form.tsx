...
export function PayoutSettingsForm({ initial }: { initial: { method: string | null; email: string; currency: string; threshold: number } }) {
  ...
  return (
    <form className="overflow-hidden rounded-2xl border border-border bg-card" onSubmit={async (event) => {
      ...
      try {
        const response = await fetch("/api/account/payout", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ method, email, currency, threshold, currentPassword }) });
        const data = await response.json();
        if (!response.ok) { setError(data.error ?? "Could not save payout settings"); setStatus("idle"); return; }
        setCurrentPassword(""); setStatus("saved");
      ...
      <div className="space-y-6 p-5 sm:p-7">
        ...
        <div className="grid gap-5 md:grid-cols-2"><Field id="payout-threshold" label="Minimum payout" as="select" required value={String(threshold)} onChange={(event) => { setThreshold(Number(event.target.value)); setStatus("idle"); }}><option value="25">25</option><option value="50">50</option><option value="100">100</option><option value="250">250</option></Field><Field id="payout-current-password" label="Current password" type="password" autoComplete="current-password" required value={currentPassword} onChange={(event) => { setCurrentPassword(event.target.value); setStatus("idle"); }} helper="Required to protect payout destination changes." /></div>
