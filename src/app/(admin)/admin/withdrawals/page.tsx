...
export default async function WithdrawalsPage({ searchParams }: Props) {
  ...
  const [rows, total, statusGroups, methodGroups, pendingWithdrawals] = await Promise.all([
    ...
    prisma.withdrawal.findMany({ where: { status: "pending" }, select: { amount: true, paidAmount: true } }),
  ]);
  const count = (status: WalletTransactionStatus) => statusGroups.find((group) => group.status === status)?._count ?? 0;
  const pendingNetPayable = pendingWithdrawals.reduce(
    (total, withdrawal) => total + Number(withdrawal.paidAmount ?? withdrawal.amount),
    0,
  );
