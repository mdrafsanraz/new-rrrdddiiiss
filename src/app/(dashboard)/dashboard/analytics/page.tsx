...
export default async function AnalyticsPage({ searchParams }: { searchParams: Search }) {
  ...
  let analytics: Awaited<ReturnType<typeof getReleaseAnalytics>> | null = null;
  let loadError: string | null = null;
  if (selected?.labelgridId && isLabelGridLive()) {
    try { analytics = await getReleaseAnalytics({ labelgridReleaseId: Number(selected.labelgridId), startDate: dateOnly(start), endDate: dateOnly(end), platform }); }
    catch (error) { console.error("[dashboard/analytics] LabelGrid fetch failed", error); loadError = "Analytics data is temporarily unavailable. Please try again shortly."; }
  }
  ...
  const countries = rows(sections["streams-by-country"]).map((row) => ({ name: text(row, ["country_name", "country", "name", "country_code"]) || "Unknown", value: metric(row, ["streams", "total", "count", "value"]) })).sort((a, b) => b.value - a.value).slice(0, 6);
  const countryMax = Math.max(...countries.map((country) => country.value), 1);
  const leaders = (analytics?.leaders ?? []).map((value) => value as UnknownRow).slice(0, 6);
  const placements = (analytics?.placements ?? []).map((value) => value as UnknownRow).slice(0, 6);

  return <div className="mx-auto max-w-[1200px] space-y-6">
    <div className="flex flex-wrap items-end justify-between gap-5 border-b border-border pb-6"><div><p className="font-mono text-[10px] font-semibold uppercase tracking-[0.2em] text-primary">Audience signal</p><h1 className="mt-2 text-3xl font-semibold tracking-[-0.04em]">Analytics</h1><p className="mt-2 text-sm text-muted-foreground">Performance reported directly by distribution platforms.</p></div><form className="flex flex-wrap gap-2" action="/dashboard/analytics"><select name="release" defaultValue={selected?.id} className="h-10 min-w-48 rounded-lg border border-border bg-card px-3 text-sm outline-none focus:border-primary">{releases.map((release) => <option key={release.id} value={release.id}>{release.title}</option>)}</select><select name="platform" defaultValue={platform ?? ""} className="h-10 rounded-lg border border-border bg-card px-3 text-sm outline-none focus:border-primary"><option value="">All platforms</option>{ANALYTICS_PLATFORMS.map((item) => <option key={item} value={item}>{platformLabel(item)}</option>)}</select><select name="range" defaultValue={String(days)} className="h-10 rounded-lg border border-border bg-card px-3 text-sm outline-none focus:border-primary"><option value="28">Last 28 days</option><option value="90">Last 90 days</option><option value="365">Last year</option></select><button className={cn(buttonVariants(), "h-10 px-4")} type="submit">Apply</button></form></div>
    {!selected ? <Card><EmptyState icon={<Disc size={22} />} title="No distributed releases yet" description="Analytics become available after a release is created on LabelGrid." /></Card> : !isLabelGridLive() || loadError ? <Card><EmptyState icon={<ChartLineUp size={22} />} title="Analytics unavailable" description={loadError ?? "LabelGrid analytics is not configured in this environment."} /></Card> : <>
