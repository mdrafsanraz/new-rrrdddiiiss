...
export default async function AnalyticsPage({ searchParams }: { searchParams: Search }) {
  ...
  return <div className="mx-auto max-w-[1200px] space-y-6">
    ...
    {!scopedReleases.length ? <Card><EmptyState icon={<Disc size={22} />} title="No distributed releases yet" description="Analytics become available after a release is created on LabelGrid." /></Card> : !isLabelGridLive() || loadError ? <Card><EmptyState icon={<ChartLineUp size={22} />} title="Analytics unavailable" description={loadError ?? "LabelGrid analytics is not configured in this environment."} /></Card> : <>
